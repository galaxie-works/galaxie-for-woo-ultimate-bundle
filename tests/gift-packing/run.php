...
foreach ( $fixtures['arrange'] as $case ) {
	...
	$first = $shape( GiftPacking::arrange( $expand( $case['candles'] ), $list ) );
	$check( 'arrange', $case['name'], $first, $case['expect'] );
	$check( 'arrange', $case['name'] . ' (same again)', $shape( GiftPacking::arrange( $expand( $case['candles'] ), $list ) ), $first );
}

foreach ( $fixtures['room'] as $case ) {
	$try = array_map( static fn( string $s ): array => $sizes[ $s ], $case['sizes'] );
	$check( 'room', $case['name'], GiftPacking::room( $boxes[ $case['box'] ], $expand( $case['candles'] ), $try ), $case['expect'] );
}
...
$time( 'fits 6 x 190g + 6 x 50g in 25 x 25 (no)', static fn() => GiftPacking::fits( $boxes['b25'], $expand( array( array( '50g', 6 ), array( '190g', 6 ) ) ) ) );
$time( 'fits 6 x 190g + 6 x 50g in 30 x 30 (yes)', static fn() => GiftPacking::fits( $boxes['b30'], $expand( array( array( '190g', 6 ), array( '50g', 6 ) ) ) ) );
$time( 'fits 12 x 50g in 21 x 21 (no)', static fn() => GiftPacking::fits( $boxes['sq21'], $expand( array( array( '50g', 12 ) ) ) ) );
$time( 'arrange 8 x 50g + 4 x 190g over 3 boxes', static fn() => GiftPacking::arrange( $expand( array( array( '50g', 8 ), array( '190g', 4 ) ) ), array( $boxes['p11'], $boxes['b14'], $boxes['sq14'] ) ) );
$time( 'summary b30', static fn() => GiftPacking::summary( $boxes['b30'], array( $sizes['50g'], $sizes['190g'] ) ) );
